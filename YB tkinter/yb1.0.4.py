...
def add_record():
    """Open edit form with all blank fields for a new row."""
    main_frame.place_forget()
...
def duplicate_record():
    """Copy selected row into edit form with PK cleared."""
...
def remove_record():
    """Delete selected row after confirmation."""
    ...
    def confirm_delete():
        db_path = get_db_path()
        try:
            db_mapper.delete_record_from_db(
                db_path, table_name, pk_col, pk_val)
...
def populate_edit_form(blank=False, duplicate=False):
    ...
    # Get values from selected row (if not blank mode)
    values = []
    if not blank:
        selected = tree.selection()
        if selected:
    ...
    for i, col in enumerate(cols):
        ...
        ctk.CTkLabel(form_inner, text=col, font=("Arial", 12),
                     text_color=TEXT_DIM, anchor="w").grid(
            row=row, column=col_pos * 2, padx=(0, 12), pady=10, sticky="w")

        val = ""
        if not blank and i < len(values):
            val = values[i]
        # Duplicate: keep all values except PK (left blank for auto-assign)
        if duplicate and i == 0:
            val = ""

        # PK field: read-only when editing existing, blank+disabled for add/duplicate
        is_pk = (i == 0)
        entry = ctk.CTkEntry(form_inner, width=480, height=34,
                             fg_color=PANEL,
                             text_color=TEXT_DIM if is_pk else TEXT,
        ...
        if is_pk and not blank and not duplicate:
            # Editing existing — show PK but lock it
            entry.insert(0, str(val))
            entry.configure(state="disabled")
        elif is_pk:
            # Add/Duplicate — show placeholder, lock it (DB assigns the PK)
...
def save_record():
    ...
    # If PK is disabled/blank → this is an INSERT (Add or Duplicate)
    ...
    try:
        ...
        status_label.configure(
            text="Record saved successfully!", text_color=ACCENT)
    except Exception as e:
